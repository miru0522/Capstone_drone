...
class _TensorRTClipFeatureExtractor:
    """Official crop-5/preprocess + TensorRT CLIP visual."""

    EXPECTED_ENGINE_SHA256 = "973557212ba947da9f50eeb2e458cc5cd6e403f83858223f32f6e881071cfb69"
    ...
    def __init__(
        ...
        if not device.startswith("cuda"):
            raise RuntimeError("TensorRT CLIP requires CUDA")
        ...
        if got_sha != self.EXPECTED_ENGINE_SHA256:
            raise RuntimeError(
                "TensorRT engine SHA mismatch: "
                f"expected={self.EXPECTED_ENGINE_SHA256}, got={got_sha}"
            )

        # 기존 runtime과 정확히 같은 official CLIP preprocess를 사용한다.
        # 모델 자체는 CPU에서 잠시 로드해 preprocess만 얻고 즉시 해제한다.
        _prepend_repo_paths(repo)
        clip_pkg = importlib.import_module("clip")

        cpu_model, self.preprocess = clip_pkg.load(
            str(clip_model),
            device="cpu",
        )
        ...
        if self._engine is None:
            raise RuntimeError(
                "TensorRT CLIP engine deserialization failed"
            )
        ...
        if self._context is None:
            raise RuntimeError(
                "TensorRT CLIP context creation failed"
            )

        self._input_idx = self._engine.get_binding_index("images")
        self._output_idx = self._engine.get_binding_index("features")

        if self._input_idx < 0 or self._output_idx < 0:
            raise RuntimeError(
                "TensorRT binding images/features not found"
            )

        if self._engine.get_binding_dtype(self._input_idx) != trt.float32:
            raise RuntimeError(
                f"unexpected TRT input dtype: "
                f"{self._engine.get_binding_dtype(self._input_idx)}"
            )

        if self._engine.get_binding_dtype(self._output_idx) != trt.float32:
            raise RuntimeError(
                f"unexpected TRT output dtype: "
                f"{self._engine.get_binding_dtype(self._output_idx)}"
            )

        pmin, popt, pmax = self._engine.get_profile_shape(
            0, self._input_idx
        )

        logger.info(
            "TensorRT CLIP visual loaded: %s, sha=%s, "
            "profile min=%s opt=%s max=%s, crop=%d",
            self.engine_path,
            got_sha,
            pmin,
            popt,
    ...
    def _crop(self, frame_bgr: np.ndarray):
        # 기존 runtime과 동일: image_crop 내부에서 BGR -> RGB 처리
        out = self.image_crop(
            frame_bgr,
            self.crop_index,
        )
    ...
    def encode_bgr(self, frames_bgr) -> np.ndarray:
        torch = self.torch

        tensors = [
            self.preprocess(self._crop(frame))
        ...
        ok = self._context.set_binding_shape(
            self._input_idx,
            tuple(x.shape),
        )
        ...
        if not ok:
            raise RuntimeError(
                f"TRT set_binding_shape failed: {tuple(x.shape)}"
            )

        out_shape = tuple(
            self._context.get_binding_shape(
                self._output_idx
            )
        )
        ...
        if out_shape != expected:
            raise RuntimeError(
                f"TRT CLIP output mismatch: "
                f"{out_shape} != {expected}"
            )
        ...
        bindings[self._input_idx] = int(
            x.data_ptr()
        )

        bindings[self._output_idx] = int(
            y.data_ptr()
        )
        ...
        if not ok:
            raise RuntimeError(
                "TRT CLIP execute_async_v2 failed"
            )

        # 기존 downstream 인터페이스는 numpy float32
        return (
            y.cpu()
            .numpy()
...
class VadCLIPScorer:
    ...
    def __init__(self):
        ...
        self.trt_engine = Path(
            os.environ.get(
                "VADCLIP_TRT_ENGINE",
                "/home/hpc/edge_vadclip_2026/trt/clip_visual_vitb16_b1_10_fp16.engine",
            )
        ).resolve()
        ...
        self.extractor = _TensorRTClipFeatureExtractor(
            self.repo,
            self.clip_model,
            self.device,
            self.crop_index,
    ...
    def _validate_paths(self) -> None:
        required = {
            ...
            "VADCLIP_TRT_ENGINE": self.trt_engine,
